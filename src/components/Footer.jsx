  ...
  return (
    <footer className="border-t border-white/[0.07] bg-ink-900 py-9">
      <div className="mx-auto flex max-w-content flex-col items-center justify-between gap-5 px-6 text-sm text-paper-faint sm:flex-row md:px-10">
        <div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-lg border border-signal/20 p-1"><img src={`${import.meta.env.BASE_URL}jklogo.png`} alt="" className="h-full w-full object-contain" /></span><span className="h-4 w-px bg-white/10" /><span>© {year} {identity.name}</span></div>
