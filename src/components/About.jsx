...
function ProfileCard() {
  return (
    <div className="about-profile relative overflow-hidden border-b border-white/[0.08] p-6 sm:p-8 lg:border-b-0 lg:border-r">
      ...
      <motion.div custom={0} variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="about-identity relative mt-9 overflow-hidden rounded-2xl border border-white/[0.09] bg-ink-900/65 p-6">
        ...
        <div className="flex items-center gap-5">
          <div className="about-monogram grid h-20 w-20 shrink-0 place-items-center rounded-2xl border border-signal/25 bg-signal/[0.055]">
            <span className="font-display text-3xl font-medium tracking-[-0.05em] text-paper">JK</span>
          </div>
          <div>
      ...
      <motion.blockquote custom={1} variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="relative mt-5 overflow-hidden rounded-2xl border border-signal/20 bg-signal/[0.045] p-6">
    ...
    <section id="about" className="section-shell relative overflow-hidden border-t border-white/[0.06] py-24 md:py-36">
      ...
      <div className="relative mx-auto max-w-content px-6 md:px-10">
        <div className="grid gap-8 lg:grid-cols-[1fr_390px] lg:items-end">
          ...
          <motion.div variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-70px' }} className="flex items-center gap-4 border-l border-white/[0.1] pl-5">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-signal/20 bg-signal/[0.07] text-signal"><TbTopologyStar3 className="h-5 w-5" /></span>
        ...
        <div className="about-strengths mt-5 grid overflow-hidden rounded-2xl border border-white/[0.08] bg-ink-800/55 sm:grid-cols-2 lg:grid-cols-5">
          {about.distinctive.map((line, index) => {
            ...
            return (
              <motion.article key={line} custom={index} variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-45px' }} className="about-strength relative border-b border-white/[0.07] p-5 sm:border-r lg:border-b-0">
                <span className="grid h-10 w-10 place-items-center rounded-xl border border-system/20 bg-system/[0.055] text-system"><Icon className="h-5 w-5" /></span>
