    ...
    <motion.header
      ...
      <nav
        ...
        <a href="#top" className="group flex items-center gap-3 font-display text-base font-medium tracking-wide text-paper">
          <span className="nav-mark relative grid h-10 w-10 place-items-center overflow-hidden rounded-xl border border-signal/20 p-1.5 transition-all group-hover:border-signal/45">
            <img src={`${import.meta.env.BASE_URL}jklogo.png`} alt="" className="relative h-full w-full object-contain" />
          </span>
          <span className="hidden sm:block">
            <span className="block text-sm leading-none text-paper">Jaimin Katva</span>
            <span className="mt-1.5 flex items-center gap-1.5 font-mono text-xs font-normal leading-none tracking-wide text-paper-faint">
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="absolute inset-x-0 top-[calc(100%+8px)] overflow-hidden rounded-2xl border border-white/[0.1] bg-ink-900/95 p-2 shadow-2xl backdrop-blur-xl md:hidden">
                ...
                {LINKS.map((link, index) => (
                  <li key={link.href}>
                    <a
                      ...
                      className="flex items-center justify-between rounded-xl px-4 py-3 transition-colors hover:bg-white/[0.04] hover:text-paper">
